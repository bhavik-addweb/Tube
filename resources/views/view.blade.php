		...
		<div class="modal fade" id="addplay" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
              <div class="modal-content">
				...
				<form method="post" action="{{ route('myplaylist.store' )}}" >
						@csrf
						<h5>Playlist Name</h5><select name="pid">
				@foreach($result2 as $input)
				<option class="option" value=" {{ $input->id }}">{{ $input->play_name }}.</option>
				@endforeach
				</select>
				</br>
				</br>
				  ...
				  <!-- comment form -->
				  					<h3>leave a comment</h3>
									   ...
									   <form method="post" action="{{ route('com.store' )}}" >
										@csrf
										<input type="hidden" class="textbox" name="vid" value="{{ $data->id }}">
										<input type="hidden" class="textbox" name="uid"value="{{ Auth::user()->id }}">
										<input type="text" class="textbox" value="{{ Auth::user()->name }}"disabled >
										<input type="text" class="textbox" value="{{ Auth::user()->email }}" disabled>
										
										<textarea placeholder="write comment:" name="comment" required></textarea>	
